...
class AlexNet(tf.keras.Model):
  ...
  def __init__(self, outputs):
    super().__init__()
    self.conv1 = tf.keras.layers.Conv2D(11, 96, input_shape=(224, 224, 3), strides=4)
    self.pool1 = tf.keras.layers.MaxPool2D(3, strides=2)
    self.conv2 = tf.keras.layers.Conv2D(5, 4)
    self.pool2 = tf.keras.layers.MaxPool2D(3)
    self.conv3 = tf.keras.layers.Conv2D(3, 1)
    self.conv4 = tf.keras.layers.Conv2D(3, 1)
    self.conv5 = tf.keras.layers.Conv2D(3, 1)
    self.pool3 = tf.keras.layers.MaxPool2D(3)
    self.flatten = tf.keras.layers.Flatten()
    self.dense2 = tf.keras.layers.Dense(150, activation=tf.nn.relu)
    self.dense4 = tf.keras.layers.Dense(outputs)
